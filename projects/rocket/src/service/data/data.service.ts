...
export class RocketDataService {
  ...
  public create({
    ...
    if (!force && this.isReservedName(name)) {
      console.log(
        'ROCKET DATA CREATE:',
        `The data key name "${name}" that you submitted is reserved and therefore invalid. Please use a different name.`
      );
      return;
    }
    if (this.exists(name)) {
      console.log(
        'ROCKET DATA CREATE:',
        `The data key name "${name}" that you submitted already exists. Rather use the update method to update the existing data.`
      );
      return;
    }
   ...
   *
   * @param params - The deconstructed options object.
   * @param params.name - A single instance or list of named data observables.
   * @param params.onEmit - Call a function that will handle the data each time it is emitted to this subscriber.
   * @param params.safeEmit - Only call the onEmit function if there is actual data.
   */
  public getSubscriptionFromOptions({
    ...
    if (name.length === 1) {
      return name[0]
        .pipe(
          filter((response: any) => (safeEmit ? response != undefined : true))
        )
      ...
      data: observableNames.map((item: any) => {
        ...
        if (RocketIs.string(item)) {
          ...
          if (!this.exists(item) && !this.isReservedName(item)) {
            console.log(
              'ROCKET DATA OBSERVABLE:',
              `No observable data with name "${item}" was found, so one has been created. This is not ideal. Create the observable data ahead of time instead.`
            );
   ...
   *
   * @param params - The deconstructed options object.
   * @param params.data - The data to sort.
   * @param params.sortBy - Sort the data by a property.
   * @param params.sortOrder - The order to sort the data.
   */
  private sortData({
  ...
  public update({
    ...
    if (!this.exists(name)) {
      console.log(
        'ROCKET DATA UPDATE:',
        `No data with name "${name}" was found, so one has been created`
      );
