...
import { Pane, PaneStateChangeParams } from '../../model/pane.model';
...
export class RocketPaneService {
   ...
   * @param params.active - The active state.
   * @param params.name - The name of the pane.
   */
  private changeState({ active = false, name }: PaneStateChangeParams): void {
    ...
    if (panes.has(name)) {
      ...
      if (active) {
        ...
        namedPane.level =
          this.rocketConfig.paneStartingLevel + this.activePaneLevel;
      ...
      console.log(
        Prefix.LOG,
        `A pane with the name "${name}" cannot be found thus the active state cannot be updated.`
      );
    }
  }
