...
import { IsImageParams } from '../../model/is.model';
 ...
 * @param params.allowedTypes - The allowed type to check against.
 * @param params.check - The image string to check.
 */
function isImage({
  allowedTypes = imageExtensionList,
  check
}: IsImageParams): boolean {
  /*
   * Make sure the allowed types is an array. This caters for user defined
   * lists. If not fail the check.
   */
  if (
    !isArray(allowedTypes) ||
    check === undefined ||
    check === null ||
    check.length < 1
  ) {
